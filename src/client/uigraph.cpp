...
GraphData* UIGraph::getGraph(size_t index, bool createMissing)
{
	if (index == 0)
		index = 1;

	if (createMissing) {
		while (m_graphs.size() < index)
			createGraph();
	}

	if (m_graphs.size() < index)
		return nullptr;

	return &m_graphs[index - 1];
}

void UIGraph::setLineWidth(size_t index, int width) {
	auto graph = getGraph(index, true);
	if (!graph) {
